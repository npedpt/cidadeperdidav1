			...
			if distance2 <= 1.5 and pegando and not IsPedInAnyVehicle(ped) then
			    if model == 1747439474 and GetVehicleNumberPlateText(veh) == vRP.getRegistrationNumber() then
                    ...
                    if IsControlJustPressed(0,38) and andamento then
                        ...
                        if quantidade == 15 then
                        	TriggerEvent("Notify","importante","Colocou <b>"..quantidade.."/15 Sacos de Dinheiro</b> no <b>Carro Forte</b>, entre nele e va ate os <b>Caixas Eletronicos</b>!")
                        	vRP.playSound("5_SEC_WARNING","HUD_MINI_GAME_SOUNDSET")
                        	portaaberta = false
                        	parte = 2
                        else
                            vRP.playSound("5_Second_Timer","DLC_HEISTS_GENERAL_FRONTEND_SOUNDS")
                            TriggerEvent("Notify","importante","Colocou <b>"..quantidade.."/15 Sacos de Dinheiro</b> no <b>Carro Forte</b>!")
                        end
					end
				end
			end
				...
				if model == 1747439474 and GetVehicleNumberPlateText(veh) == vRP.getRegistrationNumber() then
                    if IsControlJustPressed(0,58) and not andamento then
                        if quantidade > 0 then
                            ...
                            TriggerEvent("Notify","importante","Retirou <b>1x Saco de Dinheiro</b> do <b>Carro Forte</b>, atualmente com <b>"..quantidade.."/15 Sacos de Dinheiro</b>!")
                            vRP._CarregarObjeto("anim@heists@narcotics@trash","walk","prop_money_bag_01",50,57005,0.49,-0.18,0.00,25.0,270.0,180.0,2)
                            TirarCaixa()
                            andamento = true
                        else
                            parte = 1
                            vRP.playSound("5_SEC_WARNING","HUD_MINI_GAME_SOUNDSET")
                            TriggerEvent("Notify","aviso","Entregou todoo seu <b>estoque</b>, volte para a central e pegue mais <b>Sacos de Dinheiro</b>!")
                            quantidade = 0
						end
					end
				end
			...
			if distance <= 1.5 and not IsPedInAnyVehicle(ped) and retirando and entregando and portaaberta2 then
				if model == 1747439474 and GetVehicleNumberPlateText(veh) == vRP.getRegistrationNumber() then
                    ...
                    if IsControlJustPressed(0,58) and andamento then
				        ...
				        FreezeEntityPosition(ped,false)
				        ClearPedTasksImmediately(ped)
				        TriggerEvent("Notify","importante","Colocou o <b>Saco de Dinheiro</b> no <b>Carro Forte</b>, atualmente com <b>"..quantidade.."/15 Sacos de Dinheiro</b>!")
				        vRP.playSound("5_Second_Timer","DLC_HEISTS_GENERAL_FRONTEND_SOUNDS")
					end
				end
			end
			for i = 1,20 do
	            ...
	            elseif distancecaixa <= 50 and not entregando and not retirando then
				    ...
				    if distancecaixa < 1.5 and not IsPedInAnyVehicle(ped) then
				    	DrawMarker(21,entregalocal[i].x,entregalocal[i].y,entregalocal[i].z-0.6,0,0,0,0.0,0,0,0.5,0.5,0.4,65,105,255,50,0,0,0,1)
					    DrawText3Ds(entregalocal[i].x,entregalocal[i].y,entregalocal[i].z+0.35,"~w~Retirar o Saco de Dinheiro do ~b~Carro Forte")
					end
                end
            end
		end
...
AddEventHandler('vrp_gruppe6:startanim', function(id)
    ...
    TriggerEvent("Notify","sucesso","<b>Saco de Dinheiro</b> entregue, ganhou <b>€"..pagamento.."</b>, faltam entregar <b>"..quantidade.."/15 <b>Sacos de Dinheiro</b>!")
			...
			if model == 1747439474 and GetVehicleNumberPlateText(veh) == vRP.getRegistrationNumber() then
                ...
                elseif traseira and not portaaberta then
	                ...
	                elseif parte == 2 and not entregando then
	            	    DrawText3Ds(coordsStock.x,coordsStock.y,coordsStock.z+0.95,"~b~[E] ~w~Fechar | ~b~[G] ~w~Retirar Saco de Dinheiro")
