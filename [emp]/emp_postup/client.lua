...
-----------------------------------------------------------------------------------------------------------------------------------------
local entregavan = {
	[1] = {x=6.54,y=-262.54,z=47.22}, -- 6.54,-262.54,47.22
...
local entregalocal = {
	...
	[3] = {x=97.76,y=-256.02,z=47.48}, -- 97.767463684082,-256.02099609375,47.48991394043
	[4] = {x=19.14,y=-209.67,z=52.85}, -- 19.144956588746,-209.67292785644,52.857303619384
	[5] = {x=-44.81,y=-180.49,z=54.26}, -- -44.81,-180.49,54.26
	[6] = {x=-40.96,y=-58.68,z=63.81}, -- -40.96,-58.68,63.81
	[7] = {x=-333.17,y=102.69,z=67.61}, -- -333.17279052734,102.69686889648,67.617385864258
	[8] = {x=-384.07,y=152.70,z=65.53}, -- -384.07528686524,152.70095825196,65.531341552734
	[9] = {x=-598.77,y=147.34,z=61.67}, -- -598.77,147.34,61.67
	[10] = {x=-628.04,y=169.57,z=61.15}, -- -628.04736328125,169.57447814942,61.156455993652
	[11] = {x=-620.04,y=209.05,z=74.20}, -- -620.04,209.05,74.20
	[12] = {x=386.30,y=-1010.59,z=29.41}, -- 386.30294799804,-1010.592590332,29.417938232422
	[13] = {x=495.56,y=-1822.85,z=28.86}, -- 495.56182861328,-1822.8552246094,28.869705200196
	[14] = {x=437.69,y=-1876.07,z=27.42}, -- 437.69836425782,-1876.0703125,27.426029205322
	[15] = {x=331.40,y=-1982.12,z=24.16}, -- 331.40234375,-1982.1297607422,24.16725730896
...
AddEventHandler('vrp_postup:permissao',function()
	if not emservico then
		...
		TriggerEvent("Notify","importante","Entrou em <b>Serviço</b>, pegue a <b>Carrinha</b>!")
		ColocarRoupa()
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
		...
		if emservico and parte == 0 then
			if GetDistanceBetweenCoords(GetEntityCoords(ped), -411.87,-2797.83,6.00,true) <= 20 then
		        ...
		        if GetDistanceBetweenCoords(GetEntityCoords(ped), -411.87,-2797.83,6.00,true) <= 1 then
                    if IsControlJustPressed(0,38) then	
                    	Fade(1200)
				        TriggerEvent("Notify","importante","Retirou a <b>Carrinha</b> da garagem, pegue as <b>encomendas</b> e coloque nela!")
		                spawnVan()
                        parte = 1
				    end
			    end
	...
	while true do
		...
		if emservico and parte == 2 then
				...
				if distance2 < 1.5 and not IsPedInAnyVehicle(ped) then
                    ...
                    if IsControlJustPressed(0,38) then
                    	if quantidade >= 0 then
                            ...
                            RemoveBlip(blip)
                        	if quantidade > 0 then
                        		TriggerEvent("Notify","sucesso","<b>Encomenda</b> entregue, ganhou <b>€"..pagamento.."</b>, faltam entregar <b>"..quantidade.."/10 <b>Encomendas</b>!")
                                TriggerEvent("Notify","importante","Feche as portas traseiras, entre na <b>Carrinha</b> e aguarde até receber um <b>Chamado</b>!")
                                time = math.random(20000,30000)
                        	    Wait(time)
                        	    TriggerEvent("Notify","sucesso","Chamado recebido, entre na <b>Carrinha</b>, e vá ao <b>local</b> entregar a <b>encomenda</b>!")
                        	    vRP.playSound("ATM_WINDOW","HUD_FRONTEND_DEFAULT_SOUNDSET")
                                while true do
                                ...
                                portaaberta2 = false
                                portaaberta = false
                        	    TriggerEvent("Notify","aviso","Entregou todas as suas <b>encomendas</b>, volte para a central e pegue mais!")
                            end
                        end
					end
				end
			elseif distance2 <= 50 and not pegando and not encomendapega then
				...
				if distance2 < 1.5 and not IsPedInAnyVehicle(ped) then
					DrawMarker(21,entregalocal[destino].x,entregalocal[destino].y,entregalocal[destino].z-0.6,0,0,0,0.0,0,0,0.5,0.5,0.4,65,105,255,50,0,0,0,1)
					DrawText3Ds(entregalocal[destino].x,entregalocal[destino].y,entregalocal[destino].z+0.35,"~w~ Retirar a Encomenda da ~b~Carrinha")
			    end
			end
		end
	end
		    ...
		    if model == 444171386 and GetVehicleNumberPlateText(veh) == vRP.getRegistrationNumber() then
                ...
                elseif traseira and not portaaberta then
	                ...
	                elseif parte == 2 and not encomendapega then
	            	    DrawText3Ds(coordsVan.x,coordsVan.y,coordsVan.z+0.80,"~b~[E] ~w~Fechar | ~b~[G] ~w~Retirar Encomenda")
