...
local vehicles = {
    ...
    [7] = {model = 'elegy2', name = 'Elegy2', reward = 6000}
}
-------------------------------------------------------
...
function emP.hasPermission()
    ...
    if lc_policia ~= '' and vRP.hasPermission(user_id, lc_policia) then
        TriggerClientEvent('LCNotify', source, '~r~É um policial e provavelmente não deveria estar aqui.')
        return false
    end

    if #vRP.getUsersByPermission(lc_policia) < lc_policiais then
        TriggerClientEvent('LCNotify', source, '~r~Número insuficiente de policias.')
        return false
    end
    ...
    TriggerClientEvent('LCNotify', source, '~r~Não tem permissão.')
    return false
end
