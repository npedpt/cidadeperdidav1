...
-------------- PERMISSÕES -----------------
----------------------------------------
RegisterServerEvent('tcp:permissao')
AddEventHandler('tcp:permissao', function(loadWeed)
	local user_id = vRP.getUserId(source)
	local player = vRP.getUserSource(user_id)
	if vRP.hasGroup(user_id,"TCP") then
	  TriggerClientEvent('tcp:permissao', player)
	else
	  TriggerClientEvent('chatMessage',player, "ALERTA",{255,70,50},"Você não é desse gueto.")
	end
end)

RegisterServerEvent('CV:permissao')
AddEventHandler('CV:permissao', function(loadWeed)
	local user_id = vRP.getUserId(source)
	local player = vRP.getUserSource(user_id)
	if vRP.hasGroup(user_id,"CV") then
	  TriggerClientEvent('CV:permissao', player)
	else
	  TriggerClientEvent('chatMessage',player, "ALERTA",{255,70,50},"Você não é desse gueto.")
	end
end)

RegisterServerEvent('ADA:permissao')
AddEventHandler('ADA:permissao', function(loadWeed)
	local user_id = vRP.getUserId(source)
	local player = vRP.getUserSource(user_id)
		if vRP.hasGroup(user_id,"ADA") then
			TriggerClientEvent('ADA:permissao', player)
		else
			TriggerClientEvent('chatMessage',player, "ALERTA",{255,70,50},"Você não é desse gueto.")
	end
end)