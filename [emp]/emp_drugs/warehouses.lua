...
Citizen.CreateThread(function()
	while true do
		...
		if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), 104.21949768066,6337.7036132813,31.400224685669, true) <= 2.0 then	
		...
		if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), 99.263763427734,6346.849609375,31.375881195068, true) <= 2.0 then	
			DisplayHelpText("Pressione ~g~E~s~ para colher o ~g~Cannabis")		
			if IsControlJustPressed(0, 38) then		
				DisplayHelpText("Pegando o ~g~Cannabis")
				TaskStartScenarioInPlace(GetPlayerPed(-1), "prop_human_bum_bin", 0, true)
		...
		DrawMarker(23, 96.824569702148,6338.3540039062,31.375925064086-0.9701, 0, 0, 0, 0, 0, 0, 6.0, 6.0, 5.0, 13, 232, 255, 155, 0, 0, 5, 0, 0, 0, 0)
		if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), 96.824569702148,6338.3540039062,31.375925064086, true) <= 2.0 then	
			DisplayHelpText("Pressione ~g~E~s~ para colher o ~g~Cannabis")		
			if IsControlJustPressed(0, 38) then		
				DisplayHelpText("Pegando o ~g~Cannabis")
				TaskStartScenarioInPlace(GetPlayerPed(-1), "prop_human_bum_bin", 0, true)
		...
		DrawMarker(23, 94.229049682618,6342.7690429688,31.377033233642-0.9701, 0, 0, 0, 0, 0, 0, 6.0, 6.0, 5.0, 13, 232, 255, 155, 0, 0, 5, 0, 0, 0, 0)
		if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), 94.229049682618,6342.7690429688,31.377033233642, true) <= 2.0 then	
			DisplayHelpText("Pressione ~g~E~s~ para colher o ~g~Cannabis")		
			if IsControlJustPressed(0, 38) then		
				DisplayHelpText("Pegando o ~g~Cannabis")
				TaskStartScenarioInPlace(GetPlayerPed(-1), "prop_human_bum_bin", 0, true)
		...
		if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), 113.23827362061,6360.431640625,32.307746887207, true) <= 2.0 then	
		...
		DrawMarker(27, 117.42613983154,6362.5888671875,32.407746887207-0.9701, 0, 0, 0, 0, 0, 0, 4.0, 4.0, 1.0, 13, 232, 255, 155, 0, 0, 2, 0, 0, 0, 0)
		if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), 117.42613983154,6362.5888671875,32.407746887207, true) <= 2.0 then	
			DisplayHelpText("Pressione ~g~E~s~ para processar o ~g~Cannabis")		
			if IsControlJustPressed(0, 38) then		
				DisplayHelpText("Processando o ~g~Cannabis")
				TaskStartScenarioInPlace(GetPlayerPed(-1), "PROP_HUMAN_PARKING_METER", 0, true)
...
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
-- FUNÇÃO Farm de balas
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------RegisterNetEvent('setCheckPos')
--Citizen.CreateThread(function()
--	while true do
--		Wait(0)
--		if balas == 0 then -- CAPSULA
--			DrawMarker(23, 588.27893066406,-3280.2351074219,6.3237409591675-0.9701, 0, 0, 0, 0, 0, 0, 1.0, 1.0, 1.0, 13, 232, 255, 155, 0, 0, 2, 0, 0, 0, 0)
--			if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), 1505.5209960938,6392.111328125,20.783918380737, true) <= 1.0 then	
--				DisplayHelpText("Pressione ~g~E~s~ para fabricar as ~g~Capsulas")		
--				if IsControlJustPressed(0, 38) then		
--					balas = 1
--					DisplayHelpText("Fabricando as ~g~Capsulas")
--					TaskStartScenarioInPlace(GetPlayerPed(-1), "PROP_HUMAN_PARKING_METER", 0, true)
--					Citizen.Wait(7000)
--					DisplayHelpText("Fabricou as ~g~Capsulas")
--					ClearPedTasksImmediately(GetPlayerPed(-1))
--					TriggerServerEvent('receber:Capsulas')
--				end
--			end
--		elseif balas == 1 then --POLVORA
--			DrawMarker(23, 592.17633056641,-3280.4128417969,6.0695624351501-0.9701, 0, 0, 0, 0, 0, 0, 1.0, 1.0, 1.0, 13, 232, 255, 155, 0, 0, 2, 0, 0, 0, 0)
--			if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), 1493.416015625,6390.2421875,21.257762908936, true) <= 1.0 then	
--				DisplayHelpText("Pressione ~g~E~s~ preparar a ~g~Polvora")		
--				if IsControlJustPressed(0, 38) then		
--					premisa = 2
--					DisplayHelpText("Preparando a ~g~Polvora")
--					TaskStartScenarioInPlace(GetPlayerPed(-1), "PROP_HUMAN_BUM_BIN", 0, true)
--					Citizen.Wait(6000)
--					DisplayHelpText("preparou a ~g~Polvora")
--					ClearPedTasksImmediately(GetPlayerPed(-1))
--					TriggerServerEvent('processar:Polvora')
--				end
--			end
--		end
--	end
...
RegisterNetEvent('ADA:permissao')
AddEventHandler('ADA:permissao', function()
loadWeed()
end)

RegisterNetEvent('TCP:permissao')
AddEventHandler('TCP:permissao', function()
loadMeta()
end)

RegisterNetEvent('CV:permissao')
AddEventHandler('CV:permissao', function()
loadCoke()
end)

RegisterNetEvent('Motoclub:permissao')
AddEventHandler('Motoclub:permissao', function()
loadCoke()
end)
