...
function emP.checkPayment()
	...
	if user_id then
		if vRP.tryGetInventoryItem(user_id,"wammo|WEAPON_ASSAULTSMG",quantidade[source]) then
			vRP.giveInventoryItem(user_id,"dinheirosujo",math.random(35,50)*quantidade[source])
			quantidade[source] = nil
			return true
		else
			TriggerClientEvent("Notify",source,"aviso","Precisa de <b>"..quantidade[source].."x Balas de MTAR-21</b>.")
		end
	end
end
		...
		for l,w in pairs(soldado) do
			...
			if player then
				async(function()
					...
					TriggerClientEvent('chatMessage',player,"190",{65,130,255},"Recebemos uma denuncia do tráfico de munições, verifique o ocorrido.")
