...
local entregalocal = {
	[1] = {x=8.68,y=-243.38,z=47.66}, -- 8.68,-243.38,47.66
	[2] = {x=66.41,y=-255.80,z=52.35}, -- 66.41,-255.80,52.35
	[3] = {x=105.24,y=-258.92,z=51.49}, -- 105.24729156494,-258.92218017578,51.499256134034
	[4] = {x=19.00,y=-209.85,z=52.85}, -- 19.000968933106,-209.85182189942,52.857299804688
	[5] = {x=-44.81,y=-180.49,z=54.26}, -- -44.81,-180.49,54.26
	[6] = {x=-38.00,y=-59.61,z=64.05}, -- -38.006156921386,-59.616844177246,64.05760192871
	[7] = {x=-332.92,y=102.61,z=67.61}, -- -332.9284362793,102.61269378662,67.617286682128
	[8] = {x=-383.78,y=152.68,z=65.53}, -- -383.78,152.68,65.53
	[9] = {x=-598.77,y=147.34,z=61.67}, -- -598.77,147.34,61.67
	[10] = {x=-628.46,y=169.69,z=61.14}, -- -628.4603881836,169.69212341308,61.149196624756
	[11] = {x=-620.04,y=209.05,z=74.20}, -- -620.04,209.05,74.20
	[12] = {x=479.64,y=-1735.80,z=29.15}, -- 479.64,-1735.80,29.15
	[13] = {x=495.48,y=-1823.23,z=28.86}, -- 495.48516845704,-1823.2370605468,28.869703292846
	[14] = {x=437.74,y=-1876.04,z=27.59}, -- 437.7491760254,-1876.0493164062,27.59895324707
	[15] = {x=324.34,y=-1937.30,z=25.01}, -- 324.34,-1937.30,25.01
	[16] = {x=200.18,y=-2002.28,z=18.86}, -- 200.18,-2002.28,18.86
	[17] = {x=1206.62,y=-611.15,z=66.11}, -- 1206.620727539,-611.15539550782,66.114669799804
	[18] = {x=414.46,y=-217.06,z=59.91}, -- 414.46417236328,-217.06838989258,59.9104347229
	[19] = {x=-664.49,y=-391.42,z=34.59}, -- -664.49,-391.42,34.59
	[20] = {x=-668.11,y=-971.25,z=22.34}, -- -668.11297607422,-971.25079345704,22.340858459472
	[21] = {x=109.35,y=-1090.55,z=29.30}, -- 109.3592300415,-1090.557006836,29.30377960205
	[22] = {x=301.56,y=-883.45,z=29.28} -- 301.56680297852,-883.4541015625,29.288665771484
}
-----------------------------------------------------------------------------------------------------------------------------------------
-- INICIANDO TRABALHO
-----------------------------------------------------------------------------------------------------------------------------------------
...
AddEventHandler('vrp_ubereats:permissao',function()
	if not emservico then
		TriggerEvent("Notify","importante","Iniciou seu <b>trabalho</b>, pegue a <b>Mota</b> na garagem!")
		parte = 0
		emservico = true
	end
end)
		...
		if emservico and parte == 0 then
			if GetDistanceBetweenCoords(GetEntityCoords(ped), -1611.07,-825.26,10.06,true) <= 20 then
		        ...
		        if GetDistanceBetweenCoords(GetEntityCoords(ped), -1611.07,-825.26,10.06,true) <= 1 then
                    if IsControlJustPressed(0,38) then	
                    	Fade(1200)
				        TriggerEvent("Notify","importante","Pegou a <b>Mota</b> na garagem, espere um <b>chamado</b>!")
		                spawnMoto()
		                Wait(10000)
			...
			if distance3 <= 50 and not pegando and npcoord then
				if distance3 < 1.3 and not IsPedInAnyVehicle(ped) and npcoord and GetDistanceBetweenCoords(GetEntityCoords(npc),pegarlocal[destino].x,pegarlocal[destino].y,pegarlocal[destino].z,true) <= 0.5 then
                    ...
                    if IsControlJustPressed(0,38) and not andamento and not andamento2 then
                        ...
                        npcoord = false
                        RemoveBlip(blip)
                        TriggerEvent("Notify","importante","<b>Comida</b> pegue, guarde na sua <b>Mota</b>!")
                        if numero == 1 then
                            RequestModel(GetHashKey("prop_paper_bag_01"))
                            while not HasModelLoaded(GetHashKey("prop_paper_bag_01")) do
                                Citizen.Wait(10)
				...
				if model == 55628203 and GetVehicleNumberPlateText(veh) == vRP.getRegistrationNumber() then
                    ...
                    if IsControlJustPressed(0,38) then
                        if quantidade == 0 then
                            ...
                            TriggerEvent("Notify","importante","<b>Comida</b> guardada, entre na sua <b>Mota</b> e espere receber a <b>localizaçao</b> da <b>entrega</b>!")
	...
	while true do
		...
		if emservico and parte == 2 then
			...
			if distancecar <= 1.0 and not IsPedInAnyVehicle(ped) and not pegando then
				if model == 55628203 and GetVehicleNumberPlateText(veh) == vRP.getRegistrationNumber() then
	                DrawText3Ds(coordsMoto.x,coordsMoto.y,coordsMoto.z+0.80,"~b~[E] ~w~Retirar")
                    if IsControlJustPressed(0,38) and not andamento and not andamento2 and not encomendapega then
                        if quantidade > 0 then
                            ...
                            vRP.playSound("Oneshot_Final","MP_MISSION_COUNTDOWN_SOUNDSET")
                            TriggerEvent("Notify","importante","<b>Comida</b> retirada!")
                            SetTimeout(400,function()
                        	    encomendapega = true
                            end)
                            if numero == 1 then
                            ...
                            parte = 1
                            vRP.playSound("5_SEC_WARNING","HUD_MINI_GAME_SOUNDSET")
                            TriggerEvent("Notify","aviso","Entregou todas as suas <b>comidas</b>!")
                            quantidade = 0
					    end
				    end
				end
				...
				if distance < 1.5 and not IsPedInAnyVehicle(ped) then
                    ...
                    if IsControlJustPressed(0,38) then
                    	if quantidade == 0 then
                            ...
                            TriggerEvent("Notify","sucesso","<b>Comida</b> entregue, ganhou <b>€"..pagamento.."</b>!")
                            TriggerEvent("Notify","importante","Entre na <b>Mota</b> e aguarde até receber uma <b>Entrega</b>!")
                            RemoveBlip(blip)
                            time = math.random(20000,30000)
                        	Wait(time)
                        	vRP.playSound("ATM_WINDOW","HUD_FRONTEND_DEFAULT_SOUNDSET")
                        	TriggerEvent("Notify","sucesso","Chamada recebido, vá até o <b>local</b> e pegue a <b>comida</b>!")
                            while true do
			...
			elseif distance <= 50 and not pegando and not encomendapega then
				...
				if distance < 1.5 and not IsPedInAnyVehicle(ped) then
					DrawMarker(21,entregalocal[destino2].x,entregalocal[destino2].y,entregalocal[destino2].z-0.6,0,0,0,0.0,0,0,0.5,0.5,0.4,65,105,255,50,0,0,0,1)
					DrawText3Ds(entregalocal[destino2].x,entregalocal[destino2].y,entregalocal[destino2].z+0.35,"~w~ Retirar a Comida da ~b~Mota")
			    end
			end
		end
	end
