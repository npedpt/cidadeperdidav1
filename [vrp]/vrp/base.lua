...
vRP.prepare("vRP/create_user","INSERT INTO vrp_users(whitelisted,banned) VALUES(false,false); SELECT LAST_INSERT_ID() AS id") --entradar no servidor por whitlist
		...
		if user_id then
			...
			if not vRP.isBanned(user_id) then
				 ...
				 if vRP.isWhitelisted(user_id) then
					...
					deferrals.done("base de testes para entrar precisa de dar o deu ID: "..user_id)
					TriggerEvent("queue:playerConnectingRemoveQueues",ids)
				end
			else
				deferrals.done("Foi banido da cidade.")
				TriggerEvent("queue:playerConnectingRemoveQueues",ids)
			end
		else
