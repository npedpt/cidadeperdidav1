...
local function build_entry_menu(user_id,home_name)
		...
		if huser_id then
			if huser_id == user_id then
				...
				if hplayer ~= nil then
					...
					if vRP.request(hplayer,identitys.name.." "..identitys.firstname.." estão a tocar á sua campainha, deseja abrir?",300) then
			...
			TriggerClientEvent('chatMessage',player,"ALERTA",{255,70,50},"A Campainha escolhida não foi encontrada.")
		end
	end }
	...
	menu["Comprar"] = { function(player,choice)
		...
		if not owner then
			...
			if number then
				if vRP.request(player,"Deseja comprar esta casa?",30) then
					if vRP.tryPayment(user_id,home.buy_price) then
						vRP.setUserAddress(user_id,home_name,number)
						TriggerClientEvent('chatMessage',player,"ALERTA",{255,70,50},"Adquiriu uma residência.")
					else
			...
			TriggerClientEvent('chatMessage',player,"ALERTA",{255,70,50},"Já é proprietário de uma residência.")
		end
	end,"<b>Valor:</b> "..vRP.format(parseInt(home.buy_price)) }

	menu["Vender"] = { function(player,choice)
		if vRP.request(player,"Deseja vender sua casa?",30) then
			local addresses = vRP.getUserAddress(user_id)
			local owner = false
			for k,address in pairs(addresses) do
				if address and address.home == home_name then
			...
			if owner then
				...
				TriggerClientEvent('chatMessage',player,"ALERTA",{255,70,50},"Vendeu a sua residência.")
			else
