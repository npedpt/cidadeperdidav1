		...
		if radar.shown then
			if radar.freeze == false then
				...
				if IsEntityAVehicle(e) then
					...
					local fvspeed = GetEntitySpeed(e)*10.50
					local fplate = GetVehicleNumberPlateText(e)
					radar.info = string.format("~y~Matrícula: ~w~%s   ~y~MODELO: ~w~%s   ~y~VELOCIDADE: ~w~%s KPH",fplate,fmodel,math.ceil(fvspeed))
				end
				...
				if IsEntityAVehicle(j) then
					...
					local bvspeed = GetEntitySpeed(j)*10.50
					local bplate = GetVehicleNumberPlateText(j)
					radar.info2 = string.format("~y~Matrícula: ~w~%s   ~y~MODELO: ~w~%s   ~y~VELOCIDADE: ~w~%s KPH",bplate,bmodel,math.ceil(bvspeed))
				end
			end
