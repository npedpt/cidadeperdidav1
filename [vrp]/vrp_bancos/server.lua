...
AddEventHandler('vrp_bancos:roubar', function(robb)
  ...
  if vRP.hasPermission(user_id,cfg.permission) then
	TriggerClientEvent('chatMessage', player, 'BANCO', { 255, 69, 0 }, "Policias não podem assaltar bancos, vá trabalhar.")
  else
...
AddEventHandler('vrp_bancos:hackear', function(robb)
  ...
  local cops = vRP.getUsersByPermission(cfg.permission)
  if vRP.hasPermission(user_id,cfg.permission) then
	TriggerClientEvent('chatMessage', player, 'BANCO', { 255, 69, 0 }, "Policias não podem hackear bancos, vá trabalhar.")
  else
    if #cops >= cfg.cops then
	  if banks[robb] then
		  local bank = banks[robb]
