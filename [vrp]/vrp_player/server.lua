local discord_webhook = "https://discordapp.com/api/webhooks/717497041197989955/KmJUJsXfw9hNBpTwVIf7VdI7ExXC2Pdv2xuRG4cG-7w5bTxN-WmLSu27AZ9I62QZwYa6"
local discord_webhook1 = "https://discordapp.com/api/webhooks/717496499763806251/DqXZLe-tUw7ZKeJoeDIfMLYIbjFeC-GNkJ2QklUBYWyne1z0hur5_udw5jAfmual_VT7"
local discord_webhook2 = "https://discordapp.com/api/webhooks/717496737907998731/5-EB6S6vX7EJyuURFtHzLqwybdYmQJqJwlwYhKo_qtxfFZqol_fVYz_W7vpKC53wmDHI"
local discord_webhook3 = "https://discordapp.com/api/webhooks/717496857743327333/XJMjt5ZJZyRtVge93DOJN_kVe8lyJGCkL8UVBZzCsVwMVV_0PXF8uInMjpVL24edPVh6"
local discord_webhook4 = "https://discordapp.com/api/webhooks/717496108942622801/H2S-9sRYYbo8JVHQKn6NxaiziZYHECUsGE3HKeAweE4x1bX8A-ZE9v_-kZ_n4ErWyTME"
local discord_webhook5 = "https://discordapp.com/api/webhooks/717496233203204171/TgRPM0q4c8oI1ANZIMgo4xzJLLXiU1rPB7g49lgQV10hOyv43JpLIs4sgqYzkrI6WRmw"
...
AddEventHandler("kickAFK",function()
	...
	if not vRP.hasPermission(user_id,"admin.permissao") then
		DropPlayer(source,"Foi desconectado por ficar ausente.")
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- /SEQUESTRO
...
RegisterCommand('alertas',function(source,args,rawCommand)
	...
	if vRP.hasPermission(user_id,"alertas.permissao") then
		...
		TriggerClientEvent("Notify",source,"importante","Removeu as notificações.")
	else
		vRP.addUserGroup(user_id,"Alertas")
		TriggerClientEvent("Notify",source,"sucesso","Ativou as notificações.")
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- TRATAMENTO
...
RegisterCommand('reparar',function(source,args,rawCommand)
	...
	if not vRPclient.isInVehicle(source) then
		local vehicle = vRPclient.getNearestVehicle(source,7)
		if vRP.hasPermission(user_id,"nav_mec.permissao") then
			TriggerClientEvent('cancelando',source,true)
			vRPclient._playAnim(source,false,{{"mini@repair","fixing_a_player"}},true)
			TriggerClientEvent("progress",source,30000,"reparando")
			SetTimeout(30000,function()
...
RegisterCommand('enviar',function(source,args,rawCommand)
	...
	elseif nuser_id and parseInt(args[1]) > 0 then
		if vRP.tryPayment(user_id,parseInt(args[1])) then
			...
			TriggerEvent('logs:ToDiscord', discord_webhook3 , "ENVIAR", "```Player "..user_id.." enviou dinheiro para o ID "..nuser_id.." [€]: "..args[1].."```", "https://www.tumarcafacil.com/wp-content/uploads/2017/06/RegistroDeMarca-01-1.png", false, false)
		else
...
RegisterCommand('roubar',function(source,args,rawCommand)
	...
	if nplayer then
		...
		if #policia >= 1 then
			if vRP.request(nplayer,"Está sendo assaltado, deseja passar tudo?",30) then
...
RegisterCommand('chamar',function(source,args,rawCommand)
	...
	if user_id then
		...
		elseif args[1] == "mecanico" then
			players = vRP.getUsersByPermission("chamadomec.permissao")
...
RegisterCommand('use',function(source,args,rawCommand)
	...
	elseif args[1] == "metanfetamina" then
				...
				TriggerClientEvent('cancelando',source,true)
				vRPclient._CarregarObjeto(source,"amb@world_human_drinking@beer@male@idle_a","idle_a","prop_amb_beer_bottle",49,28422)
					TriggerClientEvent("progress",source,1000,"Injetando Metanfetamina.")
					SetTimeout(10000,function()
						vRPclient._stopAnim(source,false)
						vRPclient.playScreenEffect(source,"RaceTurbo",180)
						vRPclient.playScreenEffect(source,"DrugsTrevorClownsFight",180)
...
--	if vRP.hasPermission(user_id,"polpar.permissao") then
		...
		TriggerClientEvent('chatMessage',source,"ALERTA",{255,70,50},"Precisa do item ^1Roupas ^0 no inventário para mudar de roupa.")
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- /PAYPAL
	...
	if user_id then
		if args[1] == "sacar" and parseInt(args[2]) > 0 then
			...
			if resultado >= parseInt(args[2]) then
				...
				TriggerClientEvent("Notify",source,"sucesso","Efetuou o levantamento de <b>€"..vRP.format(parseInt(args[2])).." Euros</b> da sua conta paypal.")
				vRP.logs("/savedata/paypal.txt","[ID]: "..user_id.." / [SACAR]: "..parseInt(args[2]))
				TriggerEvent('logs:ToDiscord', discord_webhook4 , "PAYPAL", "```ID "..user_id.." \nLevantou €"..args[2].."```", "https://www.tumarcafacil.com/wp-content/uploads/2017/06/RegistroDeMarca-01-1.png", false, false)
			else
		...
		elseif args[1] == "trans" and parseInt(args[2]) > 0 and parseInt(args[3]) > 0 then
			...
			if banco >= parseInt(args[3]) then
				...
				TriggerClientEvent("Notify",source,"sucesso","Enviou <b>€"..vRP.format(parseInt(args[3])).." Euros</b> ao passaporte <b>"..vRP.format(parseInt(args[2])).."</b>.")
				...
				if player == nil then
					...
					local identity = vRP.getUserIdentity(user_id)
					TriggerClientEvent("Notify",player,"importante","<b>"..identity.name.." "..identity.firstname.."</b> transferiu <b>€"..vRP.format(parseInt(args[3])).." Euros</b> para sua conta do paypal.")
				end
			else
