...
AddEventHandler("vrp_pets:abrimenucompra", function()
	...
	for k, v in pairs(pets) do
		menudata[v.name] = {function (choice)
			...
			local Certeza = vRP.request(_source, "Tem certeza que deseja <b>comprar</b> por €" .. v.price .. ",00?", 30)
			if Certeza then
			if vRP.tryPayment(player, v.price) then
				vRP.execute("vRP/comprar_pet", {pet = pets[k].funcao, id = player})
				--vRPclient.notifyPicture(_source, "CHAR_CHOP", 9, "Pet-Shop ProjetoRJ", false, "Você comprou um ~b~" .. pets[k].name .. "!~r~   Valor:~g~ € " .. v.price)
				TriggerClientEvent("Notify", _source, "sucesso", "Comprou um " .. pets[k].name .. "! Valor:€ " .. v.price)
			else 
		...
		end, "<text01>Valor:</text01><text02>€"..v.price.."</text02>"}
	end
