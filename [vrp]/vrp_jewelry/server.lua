			...
			if vRP.getInventoryItemAmount(user_id,"pendrive") >= 1 and not roubando then
				...
				SetTimeout(sec*1000,function()
					if firewall >= 50 then
						...
						TriggerClientEvent("Notify",source,"sucesso","A proteção do <b>Bolacha Antivirus</b> foi comprometida e todos os balcões foram liberados.")
						...
						func.callPolice(x,y,z)
						TriggerClientEvent('iniciandojewelry',source,x,y,z,h,sec,tipo,false)
						TriggerClientEvent("Notify",source,"aviso","O computador é protegido pelo <b>Bolacha Antivirus</b>, ele bloqueou sua conexão, tente novamente.")
					end
				end)
			else
