...
vRP._prepare("vRP/concessionaria",[[
...
vRP._prepare("vRP/inserir_veh","INSERT INTO vrp_concessionaria(modelo, nome, preco, quantidade, descricao, img) VALUES(@modelo, @nome, @preco, @quantidade, @descricao, @img)")
vRP._prepare("vRP/set_quantidade","UPDATE vrp_concessionaria SET quantidade = @quantidade WHERE id = @id")
vRP._prepare("vRP/selecionar_veh","SELECT * FROM vrp_concessionaria")
vRP._prepare("vRP/get_veh_by_id","SELECT * FROM vrp_concessionaria WHERE id = @id")

async(function()
    vRP.execute("vRP/concessionaria")
end)

local conce = cfg.concessionaria

function criarCarros()
    local rows = vRP.query("vRP/selecionar_veh")
...
AddEventHandler('get:carros', function()
    ...
    local veh = vRP.query("vRP/selecionar_veh")
    for i=1, #veh, 1 do
...
AddEventHandler('comprar:carro', function(veh)
	local source = source
    local user_id = vRP.getUserId(source)
    local rows = vRP.query("vRP/get_veh_by_id", {id = veh})
    if #rows > 0 then
        ...
        if #get_veh <= 0 then
            if quantidade > 0 then
                if vRP.tryFullPayment(user_id, valor) then
                    ...
                    vRP.execute("vRP/set_quantidade", {id = veh, quantidade = quantidade})
            ...
            TriggerClientEvent("Notify",source,"negado","<b>Já possui</b> um(a) <b>"..nome.."</b>!")
        end
	end
end)
