...
RegisterCommand('toogle',function(source,args,rawCommand)
	...
	elseif vRP.hasPermission(user_id,"paisanapolicia.permissao") then
		...
		TriggerClientEvent("Notify",source,"sucesso","Entro em serviço.")
	...
	elseif vRP.hasPermission(user_id,"paisanataxista.permissao") then
		...
		TriggerClientEvent("Notify",source,"sucesso","Entro em serviço.")
	elseif vRP.hasPermission(user_id,"lidertaxista.permissao") then
		vRP.addUserGroup(user_id,"PaisanaLTaxista")
		TriggerClientEvent("Notify",source,"sucesso","Saiu de serviço.")
	elseif vRP.hasPermission(user_id,"liderpaisanataxista.permissao") then
		vRP.addUserGroup(user_id,"Líder Taxista")
		TriggerClientEvent("Notify",source,"sucesso","Entro em serviço.")
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
...
RegisterCommand('prender',function(source,args,rawCommand)
	...
	if vRP.hasPermission(user_id,"policia.permissao") then
		...
		vRPclient.teleport(player,1711.7,2575.0,45.5)
...
AddEventHandler("vRP:playerSpawn",function(user_id,source,first_spawn)
	...
	if player then
		SetTimeout(30000,function()
			...
			if tempo > 0 then
				TriggerClientEvent('prisioneiro',player,true)
				vRPclient.teleport(player,1711.7,2575.0,45.5) -- 1711.7576904296,2575.0854492188,45.588661193848
