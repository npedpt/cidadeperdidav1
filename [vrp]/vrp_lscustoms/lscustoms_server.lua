...
function emP.checkPermission()
	...
	if user_id then
		return vRP.hasPermission(user_id,"nav_mec.permissao")
	end
end

local tbl = {
	[1] = { locked = true, player = nil },
	[2] = { locked = true, player = nil },
	[3] = { locked = true, player = nil },
	[4] = { locked = true, player = nil },
	[5] = { locked = true, player = nil },
	[6] = { locked = true, player = nil },
	[7] = { locked = true, player = nil }
}
...
AddEventHandler("LSC:finished",function(veh)
	...
	if user_id then
		...
		if vehicle and placa then
			local puser_id = vRP.getUserId(source) --vRP.getUserByRegistration(placa)
			if puser_id then
				local mods = veh.mods
				local custom = {}
