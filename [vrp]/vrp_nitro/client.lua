    ...
    while true do
        ...
        if IsControlJustPressed(0,252) and IsPedInAnyVehicle(ped, true) then
            if func.getNitro() then
                ...
                Citizen.Wait(8500)                      ---- aumentar o tempo que nitro ira durar
                SetVehicleBoostActive(carro, 0, 0)
            else
                TriggerEvent("Notify", "negado","Não possui mais nitro")    
            end
        end    
    end
end)
...
Citizen.CreateThread(function()
    while true do
        ...
        if IsPedInAnyVehicle(ped, true) then
            drawTxt(0.893,0.912,0.42,"NITRO:",200,200,200,255)
            drawTxt(0.918,0.912,0.42,nitro,255,0,0,255)
            if nitro == 0 then
                drawTxt(0.918,0.912,0.42,"0",200,200,200,255)
            end    
        end
    end
end)
