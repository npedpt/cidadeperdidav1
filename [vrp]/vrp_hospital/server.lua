...
func = {}
Tunnel.bindInterface("vrp_hospital", func)

funcCliente = Tunnel.getInterface("vrp_hospital")

vRP._prepare("vRP/update_foto",
             "UPDATE vrp_user_identities SET foto = @foto WHERE user_id = @user_id")
vRP._prepare("vRP/insert_hospital",
             "INSERT IGNORE INTO hospital (police_id,user_id,dkey,dvalue,img,valor,datahora,id_pai) VALUES (@medico_id, @user_id, @dkey, @dvalue, @img, @valor, NOW(),@id_pai)")
vRP._prepare("vRP/select",
             "SELECT p.id, p.dvalue, p.img, p.valor, DATE_FORMAT(p.datahora, '%d/%m/%Y %H:%i') datahora, u.name, u.firstname FROM hospital p INNER JOIN vrp_user_identities u ON u.user_id=p.medico_id WHERE p.user_id = @user_id AND p.dkey=@dkey ORDER BY 1 DESC LIMIT 0,20")
vRP._prepare("vRP/select_all",
             "SELECT p.id, p.dvalue, u2.foto, p.valor, DATE_FORMAT(p.datahora, '%d/%m/%Y %H:%i') datahora, u.name as nomemedico,    u.firstname as sobrenomemedico,    u2.name as nomeForagido,    u2.firstname as sobrenomeForagido FROM hospital p INNER JOIN    vrp_user_identities u ON u.user_id = p.medico_id        INNER JOIN    vrp_user_identities u2 ON u2.user_id = p.user_id WHERE p.dkey=@dkey ORDER BY 1 DESC LIMIT 0,20")

vRP._prepare("vRP/select_foragidos",
             "SELECT p.id, p.user_id, p.dvalue, u2.foto, p.valor, DATE_FORMAT(p.datahora, '%d/%m/%Y %H:%i') datahora, u.name as nomemedico,    u.firstname as sobrenomemedico,    u2.name as nomeForagido,    u2.firstname as sobrenomeForagido FROM    hospital p        INNER JOIN    vrp_user_identities u ON u.user_id = p.medico_id        INNER JOIN    vrp_user_identities u2 ON u2.user_id = p.user_id WHERE    p.dkey = 'foragido' AND u2.foragido = 1 ORDER BY 1 DESC")
...
vRP._prepare("vRP/delete_foragido",
             "DELETE FROM hospital WHERE dkey = 'foragido' AND user_id = @user_id")
...
function func.setRegistro(passaporte, dkey, dvalue, img, valor, id_pai)
    ...
    if (dkey == "foragido" and not isForagido and valor == 1) or dkey ~=
        ...
        vRP.execute("vRP/insert_hospital", {
            police_id = police_id,
            user_id = passaporte,
            dkey = dkey,
            dvalue = dvalue,
...
function func.isPolice()
    ...
    return vRP.hasPermission(user_id, "hospital.permissao")
end
