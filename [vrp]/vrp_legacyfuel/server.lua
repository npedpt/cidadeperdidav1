	...
	if user_id and price > 0 then
		if vRP.tryPayment(user_id,price) then
			if galao then
				...
				TriggerClientEvent("Notify",source,"sucesso","Pagou <b>$"..vRP.format(price).." Euros</b> pelo <b>Galão</b>.")
			else
				TriggerClientEvent("Notify",source,"sucesso","Pagou <b>$"..vRP.format(price).." Euros</b> em combustível.")
			end
		else
