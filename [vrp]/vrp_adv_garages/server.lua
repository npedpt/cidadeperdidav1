local discord_webhook1 = "https://discordapp.com/api/webhooks/717494272072351892/WXsMaazUnLdcz8_P1sFfQGeHnmbmilJmHgfqp0k5HHHYcPlpRvcmtPGSUjnHUob95kOl"
...
local carros = {
	...
	["police"] = { price = 100000 },
	["pspp_530d"] = { price = 100000 },
	["psp_bmwgs"] = { price = 100000 },
	["ambulancei"] = { price = 100000 },
	["ems_gs1200"] = { price = 100000 }
--	["rumpo3"] = { price = 15000 },
--	["benson"] = { price = 50000 },
--	["mule2"] = { price = 30000 },
--	["burrito2"] = { price = 7000 }
	...
	for _,gtype in pairs(gtypes) do
		...
		elseif gtype == "shop" then
			menu["Loja"] = { function(player,choice)
							...
							if custom then
								if vRP.tryFullPayment(user_id,0) then
									if vname then
										...
										--TriggerClientEvent("Notify",player,"sucesso","Pagou <b>€600 Euros</b>.")
									end
								else
				...
				submenu["Cores"] = { ch_color,"<text01>Valor:</text01> <text02>0</text02>" }
						...
						if custom then
							if vRP.tryFullPayment(user_id,1) then
								if vname then
									...
									TriggerClientEvent("Notify",player,"sucesso","Pagou <b>€1 Euro</b>.")
								end
							else
								vRPgarage.setVehicleMods(source,old_custom)
								TriggerClientEvent("Notify",player,"negado","Dinheiro insuficiente.")
