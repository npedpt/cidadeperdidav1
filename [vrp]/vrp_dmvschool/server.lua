...
--[[RegisterCommand("carta",function(source)
  local nplayer = vRPclient.getNearestPlayer(source,2)
	local nuser_id = vRP.getUserId(nplayer)
  local user_id = vRP.getUserId(source)
  if vRP.hasPermission(user_id,"policia.permissao") or vRP.hasPermission(user_id,"carta.permissao") then
    if nuser_id then
      ...
      --if vRP.request(nplayer,"Deseja mostrar sua carta ?",15) then
      ...
      if license and license ~= 0 then
            ...
            --if identity then
              ...
              TriggerClientEvent("Notify","sucesso","Possui carta")
              ...
              -- request to hide div
              --vRP.request(source, "Devolver carta", 1000)
              --vRPclient.removeDiv(source,"police_identity")
            --end
        else
          TriggerClientEvent("Notify",source,"negado","Não possui carta.")
          end
      else
        TriggerClientEvent("Notify",source,"negado","Não possui carta.")
        end
      --else
        --TriggerClientEvent("Notify",source,"negado","Recusou a mostrar a carta.")
      --end
    else
...
RegisterCommand("rcarta",function(source)
  local nplayer = vRPclient.getNearestPlayer(source,2)
	local nuser_id = vRP.getUserId(nplayer)
  local user_id = vRP.getUserId(source)
  if vRP.hasPermission(user_id,"policia.permissao") then
    if nuser_id then
      ...
      if license and license ~= 0 then
          ...
          TriggerClientEvent("Notify",source,"negado","Sua carta foi retirada.")
          TriggerClientEvent("Notify",source,"sucesso","carta removida com sucesso.")
        else
          TriggerClientEvent("Notify",source,"negado","Não possui carta.")
          end
      else
        ...
        TriggerClientEvent("Notify",source,"negado","Recusou mostrar a carta.")
      end
    else
      TriggerClientEvent("Notify",source,"negado","Nenhum jogador perto.")
    end
...
RegisterCommand("carta",function(source,args)
    ...
    if vRP.hasPermission(user_id,"policia.permissao") then
      ...
      if license == nil then
        TriggerClientEvent("Notify",source,"negado","Passaporte "..parseInt(args[1]).." não possui carta")
      end 
      
      if license ~= nil and license ~= "" and license ~= 0 and license ~= "null" then
        TriggerClientEvent("Notify",source,"sucesso","Passaporte "..parseInt(args[1]).." possui carta")
      end
    else
      TriggerClientEvent("Notify",source,"negado","Você não é um polícial")
    end  
...
RegisterCommand("rcarta",function(source,args)
  local user_id = vRP.getUserId(source)
  ...
  if vRP.hasPermission(user_id,"policia.permissao") then
    ...
    TriggerClientEvent("Notify",source,"sucesso","carta removida do passaporte: "..parseInt(args[1]).."")
  else    
    TriggerClientEvent("Notify",source,"negado","Você não é um polícial")
  end  
end)